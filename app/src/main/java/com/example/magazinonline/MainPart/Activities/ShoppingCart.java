...
import com.example.magazinonline.MainPart.Adapters.ShoppingCartRecyclerViewAdapter;
...
public class ShoppingCart extends AppCompatActivity {
    ...
    private TextView noProductsInShoppingCartTextView;
    ...
    // metoda ce e apelata atunci cand dam click pe butonul de inapoi
    // => activitatea curenta e finalizata
    // si mergem inapoi la activitatea situata sub aceasta in stiva
    @Override
    ...
    private void setVariables() {
        ...
        noProductsInShoppingCartTextView = findViewById(R.id.noProductsInShoppingCartTextView);
        bottomLayout = findViewById(R.id.shopping_cart_bottom_layout);
    }
    ...
    private void setProducts() {
        databaseReference.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                // verificam daca utilizatorul curent are creat un cos de cumparaturi
                // si iteram printre produsele adaugate in cos
                if (snapshot.exists() &&
                                    ...
                                    if (user.hasChild("shoppingCart")) {
                                        if (user.child("shoppingCart").hasChildren())
                                            for (DataSnapshot productID :
                                                ...
                                                // salvam cantitatea fiecarui produs
                                                // din cosul de cumparaturi din baza de date
                                                int quantity =
                                                        Integer.parseInt(String.valueOf(productID
                                                                .getValue()));

                                                // iteram printre produsele existente
                                                // din baza de date si verificam daca exista id-ul
                                                // corespunzator fiecarui produs din cos
                                                if (snapshot.hasChild("Product"))
                                                    if (snapshot.child("Product").hasChildren()) {
                                                        for (DataSnapshot product :
                                                                snapshot.child("Product")
                                                                        .getChildren())

                                                            // daca am gasit produsul in lista
                                                            // de produse din baza de date,
                                                            // il salvam si il adaugam in lista
                                                            // ce va fi afisata in recyclerview
                                                            if (String.valueOf(product.getKey())
                                                                    .equals(String
                                                                            .valueOf(productID
                                                                                    .getKey()))) {
                                                                Model model =
                                                                        product.getValue(Model.
                                                                                class);
                                                        ...
                                                        // actualizarea adapter-ului
                                                        // cu datele proaspat salvate din
                                                        // baza de date
                                                        adapter.notifyDataSetChanged();
                                                    }
                                            }
                                    }
                                    // daca utilizatorul nu are niciun produs in cos
                                    else {
                                        bottomLayout.setVisibility(View.GONE);
                                        noProductsInShoppingCartTextView
                    ...
                    if (snapshot.hasChild("User"))
                        if (snapshot.child("User").hasChildren())
                            for (DataSnapshot user :
                                ...
                                if (String.valueOf(user.getKey())
                                        .equals(currentUser.getUid()))
                                    if (user.hasChild("shoppingCart"))
                                        if (user.child("shoppingCart").hasChildren())
                                            for (DataSnapshot productID :
                                                    user.child("shoppingCart")
                                                            .getChildren()) {
                                                // salvam id-ul si cantitatea produsului curent
                                                String orderID =
                                                        String.valueOf(productID.getKey());
                                                int orderQuantity =
                                                        Integer.parseInt(String.valueOf(productID
                                                                .getValue()));

                                                // iteram printre produsele din baza de date
                                                // si ii cautam pretul produsului curent
                                                if (snapshot.hasChild("Product"))
                                                    if (snapshot.child("Product").hasChildren())
                                                        for (DataSnapshot product :
                                                                snapshot.child("Product")
                                                            ...
                                                            if (String
                                                                ...
                                                                if (product
                                                                        .hasChild("PretProdus")) {
                                                                    // salvam pretul produsului
                                                                    // curent si il adaugam
                                                                    // la suma totala
                                                                    int productPrice =
    ...
    private void payTheOrder() {
        ...
        databaseReference.child("User").child(currentUser.getUid()).child("shoppingCart")
                .removeValue();
